...
exports.default = {
  ...
  guard(m) {
    return !m.transformError && m.scratch.extractedData && m.transformIndex < m.scratch.extractedData.length && m.scratch.extractedData[m.transformIndex].decoded && typeof m.scratch.extractedData[m.transformIndex].decoded[0] === 'object' && !Array.isArray(m.scratch.extractedData[m.transformIndex].decoded[0]);
  },
  execute(m) {
  ...
  assign(m, res) {
    m.scratch.transformBuf = m.scratch.transformBuf ? Buffer.concat([m.scratch.transformBuf, res], m.scratch.transformBuf.length + res.length) : res;
