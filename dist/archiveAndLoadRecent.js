...
exports.default = {
  ...
  select: {
    ...
    guard(m) {
      return !m.selectError && !m.selectedDate && m.bounds && m.source;
    },
    execute() {
      return true;
    },
  ...
  check: {
    ...
    guard(m) {
      return !m.checkError && !m.extractDate && m.selectedDate && m.bounds && m.selectedDate < m.bounds.end && m.setupReady;
    },
    execute() {
      return true;
    },
  ...
  extract: {
    guard(m) {
      return !m.extractError && !m.scratch.extractedData && m.extractDate;
    },
    execute(m) {
  ...
  next: {
    guard(m) {
      return !m.nextError && !m.nextReady && m.scratch.extractedData && m.archiveIndex >= m.scratch.extractedData.length && m.loadReady;
    },
    execute() {
      return true;
    },
